      ...
      <section id="points-guide" className="mb-5 scroll-mt-28">
        <Card variant="elevated">
          ...
          <div className="mt-4 grid gap-2">
            {plasticRewardRates.map((item) => <div key={item.code} className="flex items-start justify-between gap-3 rounded-lg bg-slate-50 p-3"><div className="min-w-0 flex-1"><p className="text-sm font-black text-slate-900">{item.shortLabel}</p><p className="mt-0.5 text-xs leading-5 text-slate-500">{item.guidance}</p></div><Badge className="min-w-12 justify-center" variant={item.accepted ? "success" : "neutral"}>{item.rate} pts</Badge></div>)}
          </div>
